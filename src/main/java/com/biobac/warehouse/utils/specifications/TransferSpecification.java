...
public class TransferSpecification {
    public static Specification<Transfer> hasType(ComponentType type) {
        return (root, query, cb) -> type == null ? null : cb.equal(root.get("type"), type);
    }

    public static Specification<Transfer> buildSpecification(Map<String, FilterCriteria> filters, ComponentType type) {
        return (root, query, cb) -> {
            ...
            if (filters != null) {
                for (Map.Entry<String, FilterCriteria> entry : filters.entrySet()) {
                    ...
                    Path<?> path;
                    switch (field) {
                        case "fromWarehouseId" -> path = root.get("from").get("id");
                        case "toWarehouseId" -> path = root.get("to").get("id");
                        case "fromWarehouseName" -> path = root.get("from").get("name");
                        case "toWarehouseName" -> path = root.get("to").get("name");
